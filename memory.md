...
### 2. 本地零流量即時回放 (Local Zero-Data Replay)
*   **記憶體 Blob URL**：錄製完成的影片資料會被轉為臨時記憶體二維碼 Blob，利用 `URL.createObjectURL(blob)` 產生暫存網址。
*   **優點**：影片儲存在手機本地記憶體中，**完全不需要上傳雲端，因此消耗 0 流量**。即使在手機收訊不良、現場網路擁擠的比賽場館，也能**在一秒內瞬間加載完成**，完全不會有上傳或下載卡頓的問題。
*   **慢動作核心控制**：利用 HTML5 視訊元素的 `playbackRate` 特性，預設以 **4 倍慢速 (0.25x) 自動循環播放**。影片上方提供覆蓋的控制面板，允許裁判自由切換 `0.25x`、`0.5x`、`1.0x` 等不同播放速率，進行極細微的判定。

### 3. 一鍵計分與自動重置 (Seamless UX Reset)
*   為防止裁判在「看回放 ➔ 關閉影片 ➔ 點擊評分 ➔ 重新啟動相機」之間手忙腳亂，我們設計了 **自動相機重置鏈路 (Auto Reset Link)**：
*   **執行流程**：當裁判點擊任何計分按鈕（如左側 +2 爆裂）時，核心計分函數 `addScore()` 會**自動呼叫 `closeReplay()`**。這會即時暫停並隱藏 Replay 播放器、將畫面無縫拉回 Live 相機預覽預覽、並將錄影按鈕恢復為待錄製狀態。
*   **效果**：裁判每回合評分**僅需 3 擊 (Record ➔ Stop ➔ Score)**，省去所有繁瑣的多餘點擊！
